...
export function VisitIntakeForm({
  ...
  const handleFormSubmit = () => {
    ...
    // Clear validation state and trigger submission
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 space-y-6">
          ...
          <div>
            <input
              ...
              className={`w-24 border rounded-lg px-3 py-1.5 text-sm font-semibold transition ${
                fieldErrors.age
                  ? "border-red-500 bg-red-50 text-red-900 focus:ring-2 focus:ring-red-400 focus:border-red-500 ring-2 ring-red-200"
      ...
      {/* Clinical Vitals Section with Validation Highlights */}
      <div>
        <div className="flex items-center justify-between mb-2">
          <label className="text-xs font-semibold text-slate-500 block">{t("vitalsLabel")}</label>
          <span className="text-[11px] text-slate-400 font-medium">All vitals are validated prior to summary generation</span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { key: "temp", label: "Temperature (°F)", ph: "98.6", ref: tempRef, helper: "Bounds: 80.0°F - 115.0°F" },
            { key: "bp", label: "Blood Pressure (mmHg)", ph: "120/80", ref: bpRef, helper: "Format: Systolic/Diastolic" },
            { key: "pulse", label: "Pulse (bpm)", ph: "72", ref: pulseRef, helper: "Bounds: 30 - 250 bpm" },
            { key: "spo2", label: "SpO2 (%)", ph: "98", ref: spo2Ref, helper: "Bounds: 50% - 100%" },
          ].map((v) => {
            const raw = consultation.vitals[v.key as keyof typeof consultation.vitals] || "";
            const err = fieldErrors[v.key as ValidatedField];
            const st = v.key === "bp" ? bpStatus(raw) : vitalStatus(v.key as "temp" | "pulse" | "spo2", raw);

            return (
              <div key={v.key} className="space-y-1">
                <label className="text-xs font-medium text-slate-600 block">{v.label}</label>
                <input
                  ref={v.ref}
                  value={raw}
                  onChange={(e) => {
                    updateConsultation((d) => { d.vitals[v.key as keyof typeof consultation.vitals] = e.target.value; });
                    if (fieldErrors[v.key as ValidatedField]) {
                      setFieldErrors((prev) => ({ ...prev, [v.key]: null }));
                    }
                  }}
                  placeholder={v.ph}
                  className={`mt-1 w-full border rounded-lg px-3 py-2 text-sm font-semibold transition ${
                    err
                      ? "border-red-500 bg-red-50 text-red-900 focus:ring-2 focus:ring-red-400 focus:border-red-500 ring-2 ring-red-200"
                      : statusColor[st]
                  }`}
                />
                {err ? (
                  <p className="text-[11px] font-medium text-red-600 leading-snug">{err}</p>
                ) : (
                  <p className="text-[10px] text-slate-400">{v.helper}</p>
                )}
              </div>
            );
          })}
